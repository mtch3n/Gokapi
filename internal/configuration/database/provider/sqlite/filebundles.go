...
func (p DatabaseProvider) SaveFileBundle(bundle models.FileBundle) {
	...
	_, err = p.sqliteDb.Exec(`INSERT INTO FileBundles
   				 (id, NameEncrypted, userid, creationdate, EncryptedSharePassword,
   				  PasswordHash, ExpireAt, UnlimitedTime, DownloadsRemaining, UnlimitedDownloads, WindowOpenedAt, DeletedAt`+legacyColumns+`)
        			 VALUES  (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?`+legacyValues+`)
...
func (p DatabaseProvider) AcquireBundleDownload(id string, timeNow int64) bool {
	result, err := p.sqliteDb.Exec(`UPDATE FileBundles SET DownloadsRemaining = DownloadsRemaining - 1,
		WindowOpenedAt = ? WHERE id = ? AND DownloadsRemaining > 0`, timeNow, id)
	helper.Check(err)
