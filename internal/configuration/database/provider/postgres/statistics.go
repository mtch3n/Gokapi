...
func (p DatabaseProvider) GetStatTraffic() uint64 {
	var result uint64
	row := p.queryRow("SELECT Value FROM Statistics WHERE Type = $1", statIdTraffic)
	err := row.Scan(&result)
	if err != nil {
...
func (p DatabaseProvider) SaveStatTraffic(totalTraffic uint64) {
	...
	_, err := p.exec(`INSERT INTO Statistics (Type, Value) VALUES ($1, $2)
...
func (p DatabaseProvider) GetTrafficSince() (int64, bool) {
	var result int64
	row := p.queryRow("SELECT Value FROM Statistics WHERE Type = $1", statIdTrafficSince)
	err := row.Scan(&result)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return 0, false
