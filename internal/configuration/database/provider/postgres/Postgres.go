...
// buildDsn assembles the connection string that the driver expects from the
// individual connection fields
func buildDsn(dbConfig models.DbConnection) string {
	dsn := url.URL{
		Scheme: "postgres",
		Host:   dbConfig.HostUrl,
		Path:   "/" + dbConfig.DatabaseName,
	}
	if dbConfig.Username != "" || dbConfig.Password != "" {
		dsn.User = url.UserPassword(dbConfig.Username, dbConfig.Password)
	}
	query := url.Values{}
	if dbConfig.PostgresSslMode != "" {
		query.Set("sslmode", dbConfig.PostgresSslMode)
	}
...
func (p DatabaseProvider) Upgrade(currentDbVersion int) {
	...
	// A Postgres database is always created at DatabaseSchemeVersion, so there is no
	// older schema to migrate from yet.
}
...
func (p DatabaseProvider) init(dbConfig models.DbConnection) (DatabaseProvider, error) {
	...
	if p.postgresDb == nil {
		var err error
		p.postgresDb, err = sql.Open("pgx", buildDsn(dbConfig))
		if err != nil {
